...
#pragma omp parallel for
    for (int i = 0; i < len - e; i += 8)
    {
        a = _mm256_loadu_ps(p1 + i);
        b = _mm256_loadu_ps(p2 + i);
        c = _mm256_mul_ps(a, b);
        _mm256_storeu_ps(p3 + i, c);
    }
    for (int i = 1; i <= e; i++)
    {
    ...
    __m256 aa, bb, cc;
    __m256 m, n = _mm256_setzero_ps();
    #pragma omp parallel for
    for (int i = 0; i < len_cut - ee; i += 8)
    {
        aa = _mm256_loadu_ps(p30 + i);
        bb = _mm256_loadu_ps(p31 + i);
        cc = _mm256_loadu_ps(p32 + i);
        m = _mm256_add_ps(aa, bb);
        n = _mm256_add_ps(m, cc);
        _mm256_storeu_ps(sum + i, n);
    }
    for (int i = 1; i <= ee; i++)
    {
    ...
    unsigned int eee = movenum % 8;
    // float *finaldata = (float *)malloc(sizeof(float) * movenum);

    for (int i = 0; i < movenum - eee; i += 8)
    {
        cc = _mm256_setzero_ps();
        #pragma omp parallel for
        for (int j = 0; j < pointnum; j++)
        {
            aa = _mm256_loadu_ps(point[j] + i);
            cc = _mm256_add_ps(cc, aa);
        }
        _mm256_storeu_ps(finaldata + i, cc);
    }
    for (int i = 1; i <= eee; i++)
    {
